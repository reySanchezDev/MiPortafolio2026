import {
  siAndroid,
  siAstro,
  siDart,
  siCss,
  siDocker,
  siDotnet,
  siFirebase,
  siFlutter,
  siGithubcopilot,
  siGit,
  siGoogle,
  siGooglecloud,
  siHtml5,
  siJavascript,
  siN8n,
  siPostgresql,
  siTypescript,
} from 'simple-icons';
import type { Locale, LocalizedText } from '../types/content';

export interface IconData {
  path: string;
  hex: string;
}

export type LinkKind = 'core' | 'platform' | 'ecosystem';
export type TechGroup = 'frontend' | 'automation' | 'core' | 'data';
export type TechPriority = 'hero' | 'core' | 'support';

export interface TechGlyph {
  id: string;
  label: string;
  icon?: IconData;
  iconSrc?: string;
  brandHex?: string;
  fallback?: string;
}

export interface TechNode extends TechGlyph {
  description: LocalizedText;
  x: number;
  y: number;
  size: number;
  dx: number;
  dy: number;
  group?: TechGroup;
  priority?: TechPriority;
  labelDx?: number;
  labelDy?: number;
  showOnMobile?: boolean;
}

export interface TechLink {
  from: string;
  to: string;
  kind: LinkKind;
}

type ThemeMode = 'dark' | 'light';

const FALLBACK_BRAND = '#2f7df6';
const copy = (en: string, es: string): LocalizedText => ({ en, es });

const normalizeHex = (value: string) => {
  const hex = value.replace('#', '').trim();
  if (hex.length === 3) {
    return hex
      .split('')
      .map((part) => `${part}${part}`)
      .join('');
  }

  return hex.length === 6 ? hex : FALLBACK_BRAND.slice(1);
};

const parseHex = (value: string) => {
  const hex = normalizeHex(value);

  return {
    r: Number.parseInt(hex.slice(0, 2), 16),
    g: Number.parseInt(hex.slice(2, 4), 16),
    b: Number.parseInt(hex.slice(4, 6), 16),
  };
};

const toHex = (value: number) => Math.max(0, Math.min(255, Math.round(value))).toString(16).padStart(2, '0');

const mixHex = (base: string, target: string, ratio: number) => {
  const from = parseHex(base);
  const to = parseHex(target);
  const blend = Math.max(0, Math.min(1, ratio));
  const mix = (start: number, end: number) => start * (1 - blend) + end * blend;

  return `#${toHex(mix(from.r, to.r))}${toHex(mix(from.g, to.g))}${toHex(mix(from.b, to.b))}`;
};

const luminance = (value: string) => {
  const { r, g, b } = parseHex(value);
  const channel = (sample: number) => {
    const normalized = sample / 255;
    return normalized <= 0.03928 ? normalized / 12.92 : ((normalized + 0.055) / 1.055) ** 2.4;
  };

  return 0.2126 * channel(r) + 0.7152 * channel(g) + 0.0722 * channel(b);
};

const getBrandHex = (glyph: Pick<TechGlyph, 'icon' | 'brandHex'>) => `#${glyph.icon?.hex ?? glyph.brandHex ?? FALLBACK_BRAND.slice(1)}`;

export const techNodes: TechNode[] = [
  {
    id: 'flutter',
    label: 'Flutter',
    description: copy(
      "Google's cross-platform app layer for fast mobile, web, and desktop delivery.",
      'Capa principal de producto para apps multiplataforma rápidas con UX consistente.',
    ),
    x: 70,
    y: 50,
    size: 52,
    dx: 4,
    dy: -4,
    group: 'frontend',
    priority: 'hero',
    icon: siFlutter,
    showOnMobile: true,
  },
  {
    id: 'google',
    label: 'Google',
    description: copy(
      'Google ecosystem hub connecting Flutter, Android, Firebase, and cloud delivery.',
      'Hub del ecosistema Google que conecta Flutter, Android, Firebase y entrega cloud.',
    ),
    x: 50,
    y: 50,
    size: 52,
    dx: -2,
    dy: 1,
    group: 'core',
    priority: 'hero',
    icon: siGoogle,
    showOnMobile: true,
  },
  {
    id: 'dart',
    label: 'Dart',
    description: copy(
      'Language foundation for Flutter apps and shared code.',
      'Base de lenguaje para apps Flutter y cÃ³digo compartido.',
    ),
    x: 60,
    y: 67,
    size: 38,
    dx: 2,
    dy: -2,
    group: 'core',
    priority: 'support',
    icon: siDart,
    showOnMobile: true,
  },
  {
    id: 'android',
    label: 'Android',
    description: copy(
      'Native mobile endpoint for device-centric experiences and distribution.',
      'Capa nativa mÃ³vil para experiencias centradas en dispositivo y distribuciÃ³n.',
    ),
    x: 50,
    y: 72,
    size: 40,
    dx: -2,
    dy: 2,
    group: 'core',
    priority: 'support',
    icon: siAndroid,
    showOnMobile: true,
  },
  {
    id: 'firebase',
    label: 'Firebase',
    description: copy(
      'Auth, realtime data, and backend services for fast app loops.',
      'Auth, datos en tiempo real y servicios backend para ciclos rÃ¡pidos de app.',
    ),
    x: 30,
    y: 50,
    size: 38,
    dx: -2,
    dy: 2,
    group: 'data',
    priority: 'support',
    icon: siFirebase,
    showOnMobile: true,
  },
  {
    id: 'googlecloud',
    label: 'Google Cloud',
    description: copy(
      'Cloud back end for scalable services, APIs, and infrastructure.',
      'Back end cloud para servicios escalables, APIs e infraestructura.',
    ),
    x: 40,
    y: 33,
    size: 44,
    dx: -3,
    dy: 2,
    group: 'core',
    priority: 'core',
    icon: siGooglecloud,
    showOnMobile: true,
  },
  {
    id: 'astro',
    label: 'Astro',
    description: copy(
      'Marketing and content shell with fast static delivery.',
      'Capa de marketing y contenido con entrega estática rápida.',
    ),
    x: 60,
    y: 33,
    size: 46,
    dx: 6,
    dy: -5,
    group: 'frontend',
    priority: 'core',
    icon: siAstro,
  },
  {
    id: 'html',
    label: 'HTML5',
    description: copy(
      'Semantic structure for landing pages and content modules.',
      'Estructura semántica para landing pages y módulos de contenido.',
    ),
    x: 90,
    y: 50,
    size: 44,
    dx: 5,
    dy: -4,
    group: 'frontend',
    priority: 'support',
    icon: siHtml5,
  },
  {
    id: 'css',
    label: 'CSS',
    description: copy(
      'Visual system, spacing, motion, and responsive polish.',
      'Sistema visual, espaciado, motion y acabado responsive.',
    ),
    x: 70,
    y: 85,
    size: 40,
    dx: -4,
    dy: -3,
    group: 'frontend',
    priority: 'support',
    icon: siCss,
  },
  {
    id: 'js',
    label: 'JavaScript',
    description: copy(
      'Client-side behaviors for interactions, state, and UI glue.',
      'Comportamiento del cliente para interacciones, estado y lógica de UI.',
    ),
    x: 50,
    y: 92,
    size: 46,
    dx: 4,
    dy: 2,
    labelDx: -2,
    group: 'frontend',
    priority: 'hero',
    icon: siJavascript,
  },
  {
    id: 'ts',
    label: 'TypeScript',
    description: copy(
      'Safer frontend logic with clearer contracts and maintainability.',
      'Lógica de frontend más segura con contratos claros y mejor mantenibilidad.',
    ),
    x: 30,
    y: 85,
    size: 42,
    dx: 4,
    dy: 3,
    labelDx: -2,
    labelDy: 1,
    group: 'frontend',
    priority: 'core',
    icon: siTypescript,
  },
  {
    id: 'csharp',
    label: 'C#',
    description: copy(
      'Main language for backend logic, integrations, and business rules.',
      'Lenguaje principal para lógica backend, integraciones y reglas de negocio.',
    ),
    x: 60,
    y: 67,
    size: 36,
    dx: 2,
    dy: 1,
    labelDy: 1,
    group: 'core',
    priority: 'hero',
    iconSrc: '/icons/brands/csharp.svg',
    brandHex: '512bd4',
    fallback: 'C#',
    showOnMobile: true,
  },
  {
    id: 'dotnet',
    label: '.NET',
    description: copy(
      '.NET stack for APIs, services, and production-grade backend delivery.',
      'Stack .NET para APIs, servicios y entrega backend lista para producción.',
    ),
    x: 40,
    y: 67,
    size: 48,
    dx: -3,
    dy: 2,
    labelDy: 2,
    group: 'core',
    priority: 'hero',
    icon: siDotnet,
    showOnMobile: true,
  },
  {
    id: 'sqlserver',
    label: 'SQL Server',
    description: copy(
      'Operational database for core business data and reporting flows.',
      'Base operativa para datos de negocio y flujos de reporte.',
    ),
    x: 30,
    y: 50,
    size: 40,
    dx: 3,
    dy: 3,
    labelDx: 2,
    labelDy: 3,
    group: 'data',
    priority: 'core',
    iconSrc: '/icons/brands/sql-server.svg',
    brandHex: '0078d4',
    fallback: 'SQL',
    showOnMobile: true,
  },
  {
    id: 'postgres',
    label: 'PostgreSQL',
    description: copy(
      'Reliable relational store for structured data and service workloads.',
      'Motor relacional confiable para datos estructurados y servicios.',
    ),
    x: 10,
    y: 50,
    size: 44,
    dx: 4,
    dy: 4,
    labelDx: -2,
    labelDy: 3,
    group: 'data',
    priority: 'core',
    icon: siPostgresql,
  },
  {
    id: 'sqlite',
    label: 'SQLite',
    description: copy(
      'Lightweight local storage for embedded or offline scenarios.',
      'Almacenamiento local liviano para escenarios embebidos u offline.',
    ),
    x: 30,
    y: 15,
    size: 40,
    dx: -4,
    dy: 3,
    group: 'data',
    priority: 'support',
    iconSrc: '/icons/brands/sqlite.gif',
    brandHex: '003b57',
    fallback: 'SQL',
  },
  {
    id: 'docker',
    label: 'Docker',
    description: copy(
      'Consistent runtime packaging for local dev and deployment parity.',
      'Empaquetado consistente para desarrollo local y paridad de despliegue.',
    ),
    x: 50,
    y: 10,
    size: 44,
    dx: -4,
    dy: -2,
    group: 'automation',
    priority: 'support',
    icon: siDocker,
  },
  {
    id: 'git',
    label: 'Git',
    description: copy(
      'Version control for safe iteration, review, and release flow.',
      'Control de versiones para iterar, revisar y liberar con seguridad.',
    ),
    x: 70,
    y: 15,
    size: 38,
    dx: -3,
    dy: 2,
    group: 'automation',
    priority: 'support',
    icon: siGit,
  },
  {
    id: 'chatfuel',
    label: 'Chatfuel',
    description: copy(
      'Conversational automation layer for bots, chat experiences, and lead flows.',
      'Capa de automatización conversacional para bots, experiencias de chat y flujos de leads.',
    ),
    x: 86,
    y: 29,
    size: 50,
    dx: -1,
    dy: 1,
    group: 'automation',
    priority: 'core',
    iconSrc: '/icons/brands/chatfuel.png',
    brandHex: 'ff5d8f',
    fallback: 'CF',
    showOnMobile: true,
  },
  {
    id: 'n8n',
    label: 'n8n',
    description: copy(
      'Workflow automation for bots, integrations, and internal operational processes.',
      'Automatización de flujos para bots, integraciones y procesos operativos internos.',
    ),
    x: 40,
    y: 14,
    size: 44,
    dx: -2,
    dy: 2,
    group: 'automation',
    priority: 'core',
    icon: siN8n,
    showOnMobile: true,
  },
  {
    id: 'microsoft',
    label: 'Microsoft',
    description: copy(
      'Core ecosystem hub connecting cloud, productivity, and enterprise tools.',
      'Hub principal que conecta cloud, productividad y herramientas enterprise.',
    ),
    x: 50,
    y: 50,
    size: 48,
    dx: 1,
    dy: 1,
    group: 'core',
    priority: 'hero',
    iconSrc: '/icons/brands/microsoft.svg',
    brandHex: '00a4ef',
    fallback: 'MS',
    showOnMobile: true,
  },
  {
    id: 'azure',
    label: 'Azure',
    description: copy(
      'Cloud layer for hosting, identity, integrations, and scaling services.',
      'Capa cloud para hosting, identidad, integraciones y escalado de servicios.',
    ),
    x: 70,
    y: 50,
    size: 42,
    dx: 2,
    dy: -2,
    group: 'core',
    priority: 'hero',
    iconSrc: '/icons/brands/azure.svg',
    brandHex: '0078d4',
    fallback: 'AZ',
    showOnMobile: true,
  },
  {
    id: 'powerautomate',
    label: 'Power Automate',
    description: copy(
      'Low-code automation bridge for Microsoft-centric operational flows.',
      'Puente low-code para flujos operativos centrados en Microsoft.',
    ),
    x: 20,
    y: 72,
    size: 34,
    dx: 1,
    dy: -1,
    labelDx: 1,
    labelDy: 0,
    group: 'automation',
    priority: 'core',
    iconSrc: '/icons/brands/power-automate.svg',
    brandHex: '2d7dfa',
    fallback: 'PA',
    showOnMobile: true,
  },
  {
    id: 'outlook',
    label: 'Outlook',
    description: copy(
      'Messaging endpoint for notifications, routing, and user communication.',
      'Canal de mensajería para notificaciones, ruteo y comunicación con usuarios.',
    ),
    x: 86,
    y: 35,
    size: 34,
    dx: 1,
    dy: 1,
    labelDx: -1,
    labelDy: 1,
    group: 'automation',
    priority: 'support',
    iconSrc: '/icons/brands/outlook.svg',
    brandHex: '0078d4',
    fallback: 'OL',
    showOnMobile: true,
  },
  {
    id: 'teams',
    label: 'Teams',
    description: copy(
      'Collaboration channel for alerts, approvals, and operational visibility.',
      'Canal colaborativo para alertas, aprobaciones y visibilidad operativa.',
    ),
    x: 14,
    y: 65,
    size: 34,
    dx: 1,
    dy: 1,
    labelDx: 0,
    labelDy: 2,
    group: 'automation',
    priority: 'core',
    iconSrc: '/icons/brands/teams.svg',
    brandHex: '6264a7',
    fallback: 'TM',
    showOnMobile: true,
  },
  {
    id: 'copilot',
    label: 'Copilot',
    description: copy(
      'AI assistance for faster implementation, drafting, and technical exploration.',
      'Asistencia de IA para implementar, redactar y explorar soluciones más rápido.',
    ),
    x: 40,
    y: 33,
    size: 34,
    dx: 1,
    dy: -1,
    group: 'core',
    priority: 'support',
    fallback: 'CP',
    icon: siGithubcopilot,
    showOnMobile: true,
  },
  {
    id: 'al',
    label: 'AL',
    description: copy(
      'Business Central extension language for ERP customization and workflows.',
      'Lenguaje de extensiones para customizar ERP y flujos en Business Central.',
    ),
    x: 60,
    y: 33,
    size: 32,
    dx: 1,
    dy: 1,
    labelDx: 0,
    labelDy: -1,
    group: 'core',
    priority: 'support',
    iconSrc: '/icons/brands/al.svg',
    brandHex: '00b7c3',
    fallback: 'AL',
    showOnMobile: true,
  },
];

export const techLinks: TechLink[] = [
  // --- GOOGLE HUB ---
  { from: 'google', to: 'flutter', kind: 'core' },
  { from: 'google', to: 'dart', kind: 'core' },
  { from: 'google', to: 'android', kind: 'core' },
  { from: 'google', to: 'firebase', kind: 'core' },
  { from: 'google', to: 'googlecloud', kind: 'core' },
  { from: 'google', to: 'astro', kind: 'core' },
  { from: 'google', to: 'chatfuel', kind: 'ecosystem' },
  { from: 'googlecloud', to: 'postgres', kind: 'ecosystem' },

  // --- MICROSOFT HUB ---
  { from: 'microsoft', to: 'azure', kind: 'core' },
  { from: 'microsoft', to: 'csharp', kind: 'core' },
  { from: 'microsoft', to: 'dotnet', kind: 'core' },
  { from: 'microsoft', to: 'sqlserver', kind: 'core' },
  { from: 'microsoft', to: 'powerautomate', kind: 'core' },
  { from: 'microsoft', to: 'copilot', kind: 'core' },
  { from: 'microsoft', to: 'al', kind: 'core' },
  { from: 'microsoft', to: 'n8n', kind: 'core' },
  { from: 'microsoft', to: 'outlook', kind: 'ecosystem' },
  { from: 'microsoft', to: 'teams', kind: 'ecosystem' },

  // --- PLATFORM & LOGICAL FLOWS ---
  { from: 'flutter', to: 'dart', kind: 'platform' },
  { from: 'flutter', to: 'android', kind: 'platform' },
  { from: 'azure', to: 'dotnet', kind: 'platform' },
  { from: 'csharp', to: 'dotnet', kind: 'platform' },
  { from: 'n8n', to: 'chatfuel', kind: 'platform' },
  { from: 'al', to: 'dotnet', kind: 'ecosystem' },

  // --- TECHNOLOGY ORBITS (OUTER) ---
  { from: 'astro', to: 'html', kind: 'ecosystem' },
  { from: 'html', to: 'css', kind: 'ecosystem' },
  { from: 'css', to: 'js', kind: 'ecosystem' },
  { from: 'js', to: 'ts', kind: 'ecosystem' },
  { from: 'ts', to: 'dotnet', kind: 'ecosystem' },
  { from: 'copilot', to: 'js', kind: 'ecosystem' },

  // --- DATA & OPS ORBIT ---
  { from: 'sqlserver', to: 'postgres', kind: 'ecosystem' },
  { from: 'postgres', to: 'sqlite', kind: 'ecosystem' },
  { from: 'sqlite', to: 'docker', kind: 'ecosystem' },
  { from: 'docker', to: 'git', kind: 'ecosystem' },

  // --- PRODUCTIVITY ORBIT ---
  { from: 'powerautomate', to: 'outlook', kind: 'ecosystem' },
  { from: 'teams', to: 'outlook', kind: 'ecosystem' },
];

export const brandStackIcons: TechGlyph[] = techNodes.map(({ id, label, icon, iconSrc, brandHex, fallback }) => ({
  id,
  label,
  icon,
  iconSrc,
  brandHex,
  fallback,
}));

export const resolveTechBrand = (glyph: Pick<TechGlyph, 'icon' | 'brandHex'>) => getBrandHex(glyph);
export const resolveTechDescription = (node: Pick<TechNode, 'description'>, locale: Locale) => node.description[locale];

export const resolveTechTone = (glyph: Pick<TechGlyph, 'icon' | 'brandHex'>, theme: ThemeMode) => {
  const brand = getBrandHex(glyph);
  const toneLuminance = luminance(brand);

  if (theme === 'dark') {
    if (toneLuminance < 0.08) return mixHex(brand, '#dff5ff', 0.88);
    if (toneLuminance < 0.16) return mixHex(brand, '#f5fbff', 0.74);
    if (toneLuminance < 0.3) return mixHex(brand, '#ffffff', 0.48);
    return mixHex(brand, '#ffffff', 0.16);
  }

  if (toneLuminance > 0.86) return mixHex(brand, '#102338', 0.44);
  if (toneLuminance > 0.72) return mixHex(brand, '#102338', 0.3);
  if (toneLuminance < 0.1) return mixHex(brand, '#2f7df6', 0.12);
  return mixHex(brand, '#102338', 0.08);
};
